...
def _timeframe_kb(gid: str) -> InlineKeyboardMarkup:
    ...
    if row:
        rows.append(row)
    rows.append([InlineKeyboardButton("⬅️ Back", callback_data="menu_futures")])
...
def _action_kb(label: str, gid: str, tf_default: int = 15) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⬅️ Back", callback_data="menu_futures")],
    ])
...
# Handlers
async def futures_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    await q.edit_message_text("Futures / Stocks groups:", reply_markup=_menu_kb())
